...
int firstOccurrence(vector<int> &arr,int key)
{
    ...
    {
        //if element found, storing the index and searching in the left search space for more occurrences, if any are there before mid.
        if(arr[mid]==key)
        {
            index=mid;
            endIndex=mid-1;
...
int main()
{
    ...
    {
        cout<<"The element "<<target<<" has first occurrence at "<<index<<"th index in the array"<<endl;
    }
    return 0;
}
